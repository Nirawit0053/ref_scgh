...
Verify that Merchandise approver and super user able to reject adjustment successfully
    ...
    common_feature.Find brands of categories  ${Brand_name['DDK']}
    ...
    common.Update csv file    ${Qtm_814['csv_file'][0]}    ${Qtm_813['column']}    ${8}   ${value}
    ${data}    common.Get data from csv file    ${Qtm_814['csv_file'][0]}    ${Qtm_813['column']}    ${8}  
    ${price}    common.Get information in list    ${data}    ${14}
    ${real_price}    common.Convert value to currency    ${price}

    common.Choose csv file    ${folder_name['csv_data']}    ${Qtm_814['csv_file'][0]}
    ...
    BuiltIn.Wait until keyword succeeds    3x    2s    forecast_page.Verify adjust background color should be display color    ${Qtm_814['code']}    ${Brand_name['DDK']}    
    ...    ${real_price}    ${color['red_color']}
    SeleniumLibrary.Switch Window    ${headles}[0]
    common.Click expand notification
    forecast_page.Verify notification merchandise has reject    ${MCH2_category['Handle_knob']}
    ...
    ${expected_email}    forecast_page.Get expected notification reject    ${MCH2_category['Handle_knob']}    ${Brand_name['DDK']}
