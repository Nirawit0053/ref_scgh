...
Verify user able to re-import successfully
    ...
    common.Input article Number     ${QTM_1111['article_number']}
    ...
    common.Choose csv file    ${folder_name['csv_data']}    ${QTM_1111['csv_file']['import_file']}      ${file_extension['csv']}
    common.Verify wording after choose file
    min_max_level.Verify min max by rdc code and article number in table should be display blue color      ${QTM_1111['rdc'][0]}      ${QTM_1111['rdc'][1]}    ${QTM_1111['rdc'][2]}
    common.Download file for edit and move to csv folder    ${QTM_1111['csv_file']['export_file']}    ${Directory_download}
    ${reason}    forecast_page.Get the reason invalid file    ${QTM_1111['csv_file']['export_file']}     ${error_message['both_min_max_numeric_error']}
    BuiltIn.Should be equal    ${reason}    ${error_message['both_min_max_numeric_error']}

    common.Copy file to directory    ${QTM_1111['csv_file']['export_file']}    ${QTM_1111['csv_file']['reimport_file']}
    common.Update csv file from column and row         ${QTM_1111['csv_file']['reimport_file']}    ${QTM_1111['column_name'][0]}     ${4}      ${QTM_1111['new_value'][0]}
    common.Update csv file from column and row         ${QTM_1111['csv_file']['reimport_file']}    ${QTM_1111['column_name'][1]}     ${4}      ${QTM_1111['new_value'][1]}
    common_feature.Click discard after check adjust price
    common.Click button using name            ${button_name['btn_edit']}

    common.Choose csv file  ${folder_name['csv_data']}        ${QTM_1111['csv_file']['reimport_file']}      ${file_extension['csv']}
    common.Verify wording after choose file
