...
Verify that user able import data successfully by valid and invalid data - reimport
    ...
    common.Input article number  ${QTM_1079['article_num']}
    ...
    common.Choose csv file  ${folder_name['others']}  ${productrecom_01['others']['rdcinvalid']}   ${file_extension['csv']}
    common.Verify warning message items invalid  ${QTM_1079['iteminvalid']}
    ...
    product_recom_page.Verify unexpected notification for productrecom    ${page['productrecom']}    ${unexpected_id}    ${QTM_1079['item']}
    ...
    ...   ${page['productrecom']}   ${QTM_1079['item']}
    common.Remove the whitespace from messages then compare    ${actual_notifiction_mail}    ${expected_notification}


    common.Download file for edit and move to csv folder    ${QTM_1079['csv_file']['export_file']}    ${Directory_download}
    common.Copy file to directory    ${QTM_1079['csv_file']['export_file']}    ${QTM_1079['csv_file']['reimport_file']}

    common.Update csv file from column and row     ${QTM_1079['csv_file']['reimport_file']}    ${QTM_1079['column_name'][0]}    ${4}    ${QTM_1079['new_value'][0]}
    common_feature.Click discard after check adjust price
    common.Click button using name            ${button_name['btn_edit']}
    common.Choose csv file  ${folder_name['csv_data']}        ${QTM_1079['csv_file']['reimport_file']}      ${file_extension['csv']}
